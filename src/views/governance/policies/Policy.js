...
const Policy = () => {
  ...
  const fetchPolicyDetail = async policyId => {
    ...
    try {
      ...
      try {
        console.log('🏨 Fetching policy detail via proxy...', policyId)
        response = await api.get(`/governance/policy/${policyId}`)
      } catch {
        console.log('⚠️ Proxy failed, trying direct API...')
        response = await directApi.get(`/governance/policy/${policyId}`)
      }

      console.log('✅ Policy detail API response:', response.data)
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      const response = id
        ? await api.patch(`/governance/policy/${id}`, payload)
        : await api.post('/governance/policy', payload)
          ...
          <CCard>
            <CCardHeader>
              <div className="d-flex justify-content-between align-items-center">
                <h4 className="mb-0">{id ? 'Policy Detail' : 'Create New Policy'}</h4>
                <CButton color="secondary" variant="outline" onClick={() => navigate('/governance/policies')}>
