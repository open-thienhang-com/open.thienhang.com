...
export class NotificationService {
  ...
  getAuditLogs(params: any = {}): Observable<ApiResponse<ListAuditResponse>> {
    return this.http.post<any>(`${this.getBaseUrl()}/audit`, params, { headers: this.getHeaders() }).pipe(
      map(resp => this.normalizeResponse<ListAuditResponse>(resp))
    );
  }

  sendNotification(payload: any): Observable<ApiResponse<any>> {
    return this.http.post<any>(`${this.getBaseUrl()}/send`, payload, { headers: this.getHeaders() }).pipe(
      map(resp => this.normalizeResponse<any>(resp))
    );
  }
