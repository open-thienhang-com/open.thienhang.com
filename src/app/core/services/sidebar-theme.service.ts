...
export interface SidebarThemeState {
  preset: string;
  primary: string;
  surface: string | null;
  darkTheme: boolean;
...
const DEFAULT_STATE: SidebarThemeState = { preset: 'Aura', primary: 'noir', surface: null, darkTheme: false };
...
export class SidebarThemeService {
  ...
  themeState = signal<SidebarThemeState>(this.loadState());
  ...
  primaryColors = computed(() => {
    const palette = PRESETS[this.themeState().preset]?.primitive ?? {};
    const colors = ['emerald','green','lime','orange','amber','yellow','teal','cyan','sky','blue','indigo','violet','purple','fuchsia','pink','rose'];
    return [{ name: 'noir', palette: {} }, ...colors.map(c => ({ name: c, palette: palette[c] ?? {} }))];
  });

  get ripple() { return this.config.ripple(); }
  set ripple(v: boolean) { this.config.ripple.set(v); }

  constructor() {
    effect(() => {
      const state = this.themeState();
      this.saveState(state);
      this.applyDarkMode(state);
    });
  ...
  onPresetChange(preset: string): void {
    ...
    $t().preset(p).preset(this.getPresetExt()).surfacePalette(surfacePalette).use({ useDefaultOptions: true });
  }

  updateColors(event: Event, type: 'primary' | 'surface', color: any): void {
    if (type === 'primary') {
  ...
  private applyDarkMode(state: SidebarThemeState): void {
  ...
  private loadState(): SidebarThemeState {
    if (isPlatformBrowser(this.platformId)) {
      try {
  ...
  private saveState(state: SidebarThemeState): void {
    if (isPlatformBrowser(this.platformId)) {
